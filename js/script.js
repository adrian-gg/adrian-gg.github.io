$(document).ready(function(){
    ...
    function cargarPagina(pagina){
      $.ajax({
        ...
        success: function(data){
          ...
          setTimeout(() => {
            if(pagina == 'about' && $(".modo-cine").hasClass("modo-cine--activo")) {
              let imgVec = `<svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg"><path d="M12.2929 12.2929C12.6834 11.9024 13.3166 11.9024 13.7071 12.2929L51.8909 50.4767C52.2814 50.8672 52.2814 51.5004 51.8909 51.8909C51.5003 52.2814 50.8672 52.2814 50.4767 51.8909L12.2929 13.7071C11.9024 13.3166 11.9024 12.6834 12.2929 12.2929Z"/><path d="M12.2929 51.8909C11.9024 51.5003 11.9024 50.8672 12.2929 50.4767L50.4767 12.2929C50.8672 11.9024 51.5004 11.9024 51.8909 12.2929C52.2814 12.6834 52.2814 13.3166 51.8909 13.7071L13.7071 51.8909C13.3166 52.2814 12.6834 52.2814 12.2929 51.8909Z"/><rect width="64" height="64" fill="none"></rect></svg>`
              $('.hobbie__icono-cine').empty().append(imgVec);
            }
          }, 2000)
    ...
    function setCine(remove = false) {
      ...
      if(!remove) {
        ...
        if($(".modo-cine").hasClass("modo-cine--activo")){
          imgVec = `<svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg"><path d="M12.2929 12.2929C12.6834 11.9024 13.3166 11.9024 13.7071 12.2929L51.8909 50.4767C52.2814 50.8672 52.2814 51.5004 51.8909 51.8909C51.5003 52.2814 50.8672 52.2814 50.4767 51.8909L12.2929 13.7071C11.9024 13.3166 11.9024 12.6834 12.2929 12.2929Z"/><path d="M12.2929 51.8909C11.9024 51.5003 11.9024 50.8672 12.2929 50.4767L50.4767 12.2929C50.8672 11.9024 51.5004 11.9024 51.8909 12.2929C52.2814 12.6834 52.2814 13.3166 51.8909 13.7071L13.7071 51.8909C13.3166 52.2814 12.6834 52.2814 12.2929 51.8909Z"/><rect width="64" height="64" fill="none"></rect></svg>`
          openFullscreen()
        }else{
          imgVec = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" x="0px" y="0px" viewBox="0 0 64 64" xml:space="preserve"><g><g><path d="M55.931,30.139c-0.255-0.188-0.584-0.245-0.886-0.155l-11.411,3.442v-1.577c0-0.552-0.447-1-1-1h-1.191    c2.366-1.794,3.904-4.627,3.904-7.819c0-5.415-4.405-9.819-9.819-9.819c-4.04,0-7.516,2.454-9.021,5.947    c-1.505-3.494-4.981-5.947-9.021-5.947c-5.415,0-9.819,4.405-9.819,9.819c0,3.192,1.538,6.025,3.904,7.819h-0.787    c-0.552,0-1,0.448-1,1V49.79c0,0.553,0.448,1,1,1h31.851c0.553,0,1-0.447,1-1v-7.625l11.411,3.441    c0.095,0.029,0.192,0.043,0.289,0.043c0.212,0,0.422-0.067,0.597-0.197c0.254-0.188,0.403-0.486,0.403-0.803V30.941    C56.334,30.625,56.185,30.327,55.931,30.139z M35.527,15.21c4.312,0,7.819,3.508,7.819,7.819s-3.508,7.819-7.819,7.819    s-7.819-3.508-7.819-7.819S31.216,15.21,35.527,15.21z M29.612,30.849H23.4c1.348-1.022,2.429-2.375,3.106-3.947    C27.184,28.473,28.265,29.827,29.612,30.849z M9.666,23.029c0-4.312,3.508-7.819,7.819-7.819c4.312,0,7.82,3.508,7.82,7.819    s-3.508,7.819-7.82,7.819C13.174,30.849,9.666,27.341,9.666,23.029z M41.634,48.79H11.783V32.849h29.851V48.79z M54.334,43.304    l-10.7-3.227v-4.563l10.7-3.226V43.304z"></path><circle cx="35.527" cy="23.029" r="2.144"></circle><circle cx="17.485" cy="23.029" r="2.144"></circle></g><rect fill="none" width="64" height="64"></rect></g></svg>`
          closeFullscreen()
        }
    ...
    ContenedorGeneral.on('click', '.hobbie__icono-musica', function(){
      ...
      if(abriertoReproductor == false){
        ...
        imgVec = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" x="0px" y="0px" viewBox="0 0 64 64" xml:space="preserve"><g><path d="M32,10.667c-11.641,0-21.111,9.47-21.111,21.11V42.39c0,6.035,4.909,10.944,10.943,10.944h2.246   c0.553,0,1-0.447,1-1V32.446c0-0.552-0.447-1-1-1h-2.246c-3.694,0-6.961,1.846-8.943,4.657v-4.327   c0-10.537,8.573-19.11,19.111-19.11c10.537,0,19.11,8.573,19.11,19.11v4.327c-1.982-2.812-5.249-4.657-8.944-4.657h-2.245   c-0.553,0-1,0.448-1,1v19.888c0,0.553,0.447,1,1,1h2.245c6.035,0,10.944-4.909,10.944-10.944V31.776   C53.11,20.136,43.641,10.667,32,10.667z M23.078,33.446v17.888h-1.246c-0.481,0-0.95-0.049-1.411-0.122V33.568   c0.461-0.073,0.93-0.122,1.411-0.122H23.078z M18.421,34.126v16.528c-3.243-1.344-5.532-4.541-5.532-8.265   S15.178,35.47,18.421,34.126z M40.921,51.334V33.446h1.245c0.481,0,0.951,0.049,1.412,0.123v17.642   c-0.461,0.074-0.931,0.123-1.412,0.123H40.921z M45.578,50.654V34.126c3.243,1.344,5.532,4.54,5.532,8.264   S48.821,49.31,45.578,50.654z"></path><rect fill="none" width="64" height="64"></rect></g></svg>`
        abriertoReproductor = false;
      }

      $('.hobbie__icono-musica').empty().append(imgVec);
    ...
    ContenedorGeneral.on('click', '.hobbie__icono-code', function(){
      ...
      if(ContenedorGeneral.hasClass("modo-code--activo")) {
        imgVec = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" x="0px" y="0px" viewBox="0 0 64 64" xml:space="preserve"><g><path d="M53.627,20.167c0-0.002-0.001-0.003-0.001-0.005v-8.788c0-0.553-0.447-1-1-1H11.374c-0.553,0-1,0.447-1,1v41.252    c0,0.553,0.447,1,1,1h41.252c0.553,0,1-0.447,1-1V20.172C53.626,20.17,53.627,20.169,53.627,20.167z M51.626,12.374v6.793H12.374    v-6.793H51.626z M12.374,51.626V21.167h39.252v30.459H12.374z M24.264,29.145c0,0.357-0.19,0.688-0.5,0.866l-6.89,3.977    c-0.157,0.091-0.329,0.134-0.499,0.134c-0.346,0-0.682-0.18-0.867-0.5c-0.276-0.479-0.112-1.09,0.366-1.366l5.39-3.11l-5.39-3.111    c-0.479-0.276-0.643-0.888-0.366-1.366s0.889-0.642,1.366-0.366l6.89,3.978C24.073,28.457,24.264,28.787,24.264,29.145z"/><rect fill="none" width="64" height="64"/></g></svg>`
      } else {
        imgVec = `<svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg"><path d="M12.2929 12.2929C12.6834 11.9024 13.3166 11.9024 13.7071 12.2929L51.8909 50.4767C52.2814 50.8672 52.2814 51.5004 51.8909 51.8909C51.5003 52.2814 50.8672 52.2814 50.4767 51.8909L12.2929 13.7071C11.9024 13.3166 11.9024 12.6834 12.2929 12.2929Z"/><path d="M12.2929 51.8909C11.9024 51.5003 11.9024 50.8672 12.2929 50.4767L50.4767 12.2929C50.8672 11.9024 51.5004 11.9024 51.8909 12.2929C52.2814 12.6834 52.2814 13.3166 51.8909 13.7071L13.7071 51.8909C13.3166 52.2814 12.6834 52.2814 12.2929 51.8909Z"/><rect width="64" height="64" fill="none"></rect></svg>`
      }
      $('.hobbie__icono-code').empty().append(imgVec);
